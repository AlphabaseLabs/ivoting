...
import signInQues from "../../assets/signInQues.svg";
...
function Questions({
  ...
  useEffect(() => {
    const getQuestions = async () => {
      ...
      const res = await getLoginQuestions(uuid);
    ...
    if (answers[0] != null && answers[1] != null) {
      if (answers[0].value == undefined || answers[1].value == undefined) {
        ...
        try {
          ...
          const userApproveData = await sendLoginAnswers(userId, answers);
          ...
          console.info("Voter approved", userApproveData);
          handleNext(3);
        } catch (error) {
          ...
          if (error.toString().includes("Unauthorized")) {
            setErrorMsg(translate("invalidUser"));
          }
          console.info(error);
        }
        // }
  ...
  return (
    <>
      <Box>
        ...
        <Box mt={4}>
          <FlexBox
            ...
            $direction={systemLang == "en-US" ? "start" : "end"}
          >
            <InputLabel variant="body2" align="left">
              {ques[0] && ques[0].id != null && ques[0].question}
            </InputLabel>
          </FlexBox>
        ...
        <Box mt={4}>
          ...
          >
            <InputLabel variant="body2" align="left">
              {ques[1] && ques[1].id != null && ques[1].question}
            </InputLabel>
          </FlexBox>
      ...
      >
        <Hidden lgDown>
          <img height={300} width={250} src={signInQues} alt="SignIn" />
        </Hidden>
      </Box>
    </>
  );
